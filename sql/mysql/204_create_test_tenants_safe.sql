...
-- ============================================
-- 0. 清理已有测试数据（可选 - 确保干净环境）
-- ============================================
SELECT '🧹 开始清理已有测试数据...' AS '步骤0';

-- 删除测试客户（通过phone识别）
DELETE FROM customers WHERE phone LIKE '138000000%' OR phone LIKE '139000000%';

-- 删除测试用户
DELETE FROM users WHERE username IN ('test_admin_a', 'test_admin_b');
-- superadmin保留，不删除

-- 删除测试公司
DELETE FROM company_verifications WHERE credit_code IN ('91110000TEST00001A', '91310000TEST00001B');

SELECT '✅ 清理完成' AS '步骤0结果';
...
VALUES (
  'test_admin_a',
  '$2b$10$rG7qXQxJ3F5HZ4kK9vX1NeYqQ7qYZ0yL8nK5nK5nK5nK5nK5nK5nKO',
  '公司A测试管理员',
  'manager',
  @company_a_id,
  NOW(),
...
SELECT '✅ 创建公司A管理员，username: test_admin_a, password: 123456' AS '步骤3';
...
SELECT '✅ 创建公司B管理员，username: test_admin_b, password: 123456' AS '步骤4';
...
SELECT '✅ 为公司A创建2个测试客户' AS '步骤6';

-- 公司B的测试客户（不包含mongo_id字段）
INSERT INTO customers (name, phone, contact, customer_type, status, company_id, created_at, updated_at)
VALUES 
...
WHERE cv.id IN (@company_a_id, @company_b_id) OR cv.id = 1
...
SELECT '  3. 用 superadmin 登录，应该能看到所有公司的客户' AS '测试3';
SELECT '  4. 重启后端服务器：npm run api' AS '测试4';
