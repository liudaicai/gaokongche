...
CREATE TABLE IF NOT EXISTS `document_templates` (
  ...
  `template_code` VARCHAR(64) NOT NULL UNIQUE COMMENT '模板编码（如 TPL-CONTRACT-001）',
  ...
  `is_default` TINYINT(1) DEFAULT 0 COMMENT '是否默认模板（每种类型只能有一个默认）',
  `is_system` TINYINT(1) DEFAULT 0 COMMENT '是否系统内置模板（不可删除）',
  
  -- 多租户支持
  `company_id` INT DEFAULT NULL COMMENT '所属公司ID（NULL表示系统模板）',
  `store_id` INT DEFAULT NULL COMMENT '所属门店ID（NULL表示公司级模板）',
...
CREATE TABLE IF NOT EXISTS `template_mappings` (
  `id` INT AUTO_INCREMENT PRIMARY KEY,
  `template_id` INT NOT NULL COMMENT '模板ID',
  `placeholder` VARCHAR(128) NOT NULL COMMENT '占位符名称（如 customer_name）',
  `data_path` VARCHAR(256) NOT NULL COMMENT '数据路径（如 order.customerName）',
...
INSERT INTO `document_templates` 
  (`template_code`, `name`, `type`, `content`, `description`, `status`, `is_default`, `is_system`, `company_id`, `version`, `created_at`, `updated_at`)
VALUES (
...
-- 完成提示
-- ============================================
SELECT '文档模板系统表创建完成！' AS message;
SELECT COUNT(*) AS template_count FROM document_templates WHERE is_system = 1;
